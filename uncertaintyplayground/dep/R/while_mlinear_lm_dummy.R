...
filename <- here::here("data", basename(url))

if (!file.exists(filename)) {
  download.file(url, filename, method = "curl")
}

df <- readxl::read_xls(filename)
df <- dplyr::rename(df, Y = `Concrete compressive strength(MPa, megapascals)`)
# run this both on scaled and unscaled versions
df <- scale(select(df, -Y)) %>% bind_cols(Y = df$Y)

# importing the data and setup
source("scripts/r/utilities/setup_dummy_data.R")

# importing the inference functions
source("scripts/r/utilities/inference_funs.R")

# read_csv2("https://archive.ics.uci.edu/ml/machine-learning-databases/concrete/compressive/Concrete_Data.xls")
...
max_it <- 200
hidden <- 50 # even better results with 19 hiddens (in addition to R=9)
R <- 200 # 9 with `rf` worked super well and even bit the linear model
