...
cores <- detectCores() - 1 # set the number of cores you want to use
...
max_it <- 30
R <- 50 # 9 with `rf` worked super well and even bit the linear model
...
#   foreach(l = 1:n_hiddens, .combine = 'c') %dopar% {
#     for (r in 1:sampling_R) {
#       desired_z_R[, l, r] <-
#         rnorm(
#           n = a_size,
...
# TO BE REMOVED
# sample_z_r_parallel <- function(n_hiddens, z_means, a_size = train_smp_size, sampling_R = R, desired_sd = custom_sd) {
#   # define the `z_R` array
#   desired_z_R <- foreach(l = 1:n_hiddens, .combine = 'c') %dopar% {
#     z_l <- array(NA, c(a_size, sampling_R))
#     for (r in 1:sampling_R) {
#       z_l[, r] <-
#         rnorm(
#           n = a_size,
...
#     z_l
#   }
#   desired_z_R <- array(desired_z_R, c(a_size, n_hiddens, sampling_R))
#   return(desired_z_R)
# }

clusterExport(cl = cl, varlist = c("sample_z_r", "n_z_outcomes","R", "custom_sd"))
infer_mlinear_lm <- function(data,
  ...
  the_size <- nrow(final_x_test)

  # moving the items for parallel operation
  # clusterExport(cl = cl, varlist = c("sample_z_r", "n_z_outcomes","z_t_infer", "the_size")
  clusterExport(cl, c("z_t_infer", "the_size"),envir = environment())
  ...
  if (!is.matrix(z_t_infer[[p]])) {
    ...
    sample_z_r(n_z_outcomes[p], z_means = z_t_infer[[p]], a_size = the_size)
  }
...
while (continue) {
  ...
  z_R_sep <- foreach(p = 1:length(n_z_outcomes), .combine = 'list', .multicombine = T) %dopar% {
    if (!is.matrix(z_t[[p]])) {
      z_t[[p]] <- as.matrix(z_t[[p]])
      }
      sample_z_r(n_z_outcomes[p], z_means = z_t[[p]])
      }
  
  # z_R_sep <- foreach(p = 1:length(n_z_outcomes), .combine = 'list', .multicombine = T) %do% {
  # if (!is.matrix(z_t[[p]])) {
  #   z_t[[p]] <- as.matrix(z_t[[p]])
  # }
  # sample_z_r_parallel(n_z_outcomes[p], z_means = z_t[[p]])
  # }
