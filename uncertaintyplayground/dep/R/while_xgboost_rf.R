...
max_it <- 200
hidden <- 30 # even better results with 19 hiddens (in addition to R=9)
R <- 500 # 9 with `rf` worked super well and even bit the linear model
...
while (continue) {
  ...
  y_predt <- py_to_r(modL$predict(x_comp))
  ...
  train_perf[[i]] <-
    postResample(pred = infer_xgb_rf(df_train), obs = y_train)
  valid_perf[[i]] <-
    postResample(
      pred = infer_xgb_rf(df_valid, size = valid_smp_size),
      obs = y_valid
    )
...
for (i in 1:length(subset_list)) {
  ...
  for (j in 1:length(model_list)) {
    ...
    if (grepl("em_lm_rf", model_names[j], fixed = TRUE)) {
      mod_pred <- infer_xgb_rf(sub, size = nrow(sub))
    } else {
...
# postResample(pred = infer_xgb_rf(df_valid, size = nrow(df_valid)), obs = y_valid)
...
# postResample(pred = infer_xgb_rf(df_test, size = nrow(df_test)), obs = y_test)
...
postResample(pred = infer_xgb_rf(df_train, size = nrow(df_train)), obs = y_train)
postResample(pred = predict(randomForest, x_train), obs = y_train)
