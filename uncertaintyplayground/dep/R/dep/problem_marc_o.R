...
max_it <- 200
hidden <- 20 # even better results with 19 hiddens (in addition to R=9)
...
while (continue) {
  ...
  # dnorm(y_train[1], mean = y_R[1,], sd = 1) %>% summary()

  # apply(y_R[1,], 2, function(y_estimate) {
  #     dnorm(y_train, mean = y_estimate, sd = 1)
  #   }) %>% summary()
  ...
  modL <- xgb$XGBRFRegressor(tree_method = "hist", n_estimators = 64L, verbose = 0)
  ...
  modR <- xgb$XGBRFRegressor(tree_method = "hist", n_estimators = 64L, verbose = 0)
...
# fitting the `XGboost` model
set.seed(1)
full_xgb <- xgb$XGBRFRegressor(tree_method = "hist", n_estimators = 64L, verbose = 0)
