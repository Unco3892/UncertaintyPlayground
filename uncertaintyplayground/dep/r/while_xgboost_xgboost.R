...
library(reticulate)
use_python('/home/ilia/miniconda3/envs/phd_thesis_ubuntu/bin/python3.9')
py_config()
...
# make the inference function here instead
...
max_it <- 200
hidden <- 100 # even better results with 19 hiddens (in addition to R=9)
...
#-------------------------------------------------------------------------------
# use multi-linear xgboost, it seems to work quite well!!

xgb <- import("xgboost", convert = F)
...
# https://github.com/dmlc/xgboost/issues/7619#fromHistory
...
while (continue) {
  ...
  if (i == 1) {
    ...
    # make prediction with the rf left model
    z_t <- py_to_r(modL_p$predict(x_train))
  }
  ...
  if (i == 1) {
    ...
    #   )
    #   })
    y_R <- matrix(nrow = dim(z_R)[1], ncol = dim(z_R)[3])
    for (q in 1:dim(z_R)[3]) {
      y_R[, q] <- py_to_r(modR_p$predict(z_R[, , q]))
    }
  ...
  # using a random forest for multi-variate outcomes from here
  # https://www.randomforestsrc.org/articles/getstarted.html
  # note that `Multivar` and `cbind` produce the same results in this case

  # set a `reticulate` seed
  # NOTE FOR THE FIRST ITERATION, THE FUNCTION CRASHES with case.wts (Don't Know WHY!)
  modL <- xgb$XGBRegressor(tree_method = "hist", n_estimators = 64L)

  # fitting the `XGboost` model
  modL$fit(x_comp, mod_1, sample_weight = w_comp)

  # make the prediction with the `XGboost`
  mod_1_pred <- py_to_r(modL$predict(x_comp))

  # just print it the first time as afterwards it can become disturbing
  if (i == 1) {
    cat("Left model (MultiXGboost):\n")
  }

  print(postResample(mod_1_pred, mod_1))
  ...
  modR <- xgb$XGBRegressor(tree_method = "hist", n_estimators = 64L)

  # fitting the `XGboost` model
  modR$fit(hidden_node, y_comp, sample_weight = w_comp)

  # make the prediction with the `XGboost`
  y_comp_pred <- py_to_r(modR$predict(hidden_node))

  # just print it the first time as afterwards it can become disturbing
  if (i == 1) {
    cat("Right model (XGBoost):\n")
  }

  print(postResample(y_comp_pred, y_comp))
  ...
  train_perf[[i]] <-
    postResample(pred = infer_xgb_xgb(df_train), obs = y_train)
  valid_perf[[i]] <-
    postResample(
      pred = infer_xgb_xgb(df_valid, size = valid_smp_size),
      obs = y_valid
    )
...
# creating r objects for our validation set
x_valid_py <- r_to_py(x_valid)
y_valid_py <- r_to_py(y_valid)
# Create a list of named lists to represent the evaluation set
eval_set_2 <- list(list(x_valid_py, y_valid_py))
full_xgb <- xgb$XGBRegressor(tree_method = "hist", n_estimators = 64L, early_stopping_rounds=10L) 
# simple version
#full_xgb$fit(x_train, y_train)
full_xgb$fit(x_train, y_train, eval_set=eval_set_2, verbose = 0L, sample_weight = rep(1, nrow(x_valid))) #sample_weight is not needed but just to make sure that the comparison is fair


#-------------------------------------------------------------------------------
# measuring the performance on the training and test sets
# create the variables needed for the model performances
model_list <- list(full_xgb, NA, randomForest) # we can replace our approach with an NA since it's a function
model_names <- c("simple_xgb", paste0("em_xgb_xgb_", "h", hidden, "_r", R), "simple_rf")
...
for (i in 1:length(subset_list)) {
  ...
  for (j in 1:length(model_list)) {
    mod <- model_list[[j]]
    if (grepl("em_xgb_xgb", model_names[j], fixed = TRUE)) {
      mod_pred <- infer_xgb_xgb(sub, size = nrow(sub))
    } else if (grepl("simple_xgb", model_names[j], fixed = TRUE)) {
      mod_pred <- py_to_r(mod$predict(select(sub, -Y)))
    } else {
...
# postResample(pred = predict(lm_mod_train), obs = y_train) # nolint
# postResample(pred = infer_xgb_xgb(df_train), obs = y_train)
...
# postResample(pred = infer_xgb_xgb(df_valid, size = nrow(df_valid)), obs = y_valid)
...
# postResample(pred = infer_xgb_xgb(df_test, size = nrow(df_test)), obs = y_test)
...
# randomForest <- caret::train(Y ~ ., df_train, method = "rf", trControl = trainControl(method = "none"))
# # on the training set
# postResample(pred = py_to_r(full_xgb$predict(x_train)),obs = y_train)
# postResample(pred = infer_xgb_xgb(df_train, size = nrow(df_train)), obs = y_train)
# postResample(pred = predict(randomForest, x_train), obs = y_train)
#
# # on the validation set
# postResample(pred = py_to_r(full_xgb$predict(x_valid)),obs = y_valid)
# postResample(pred = infer_xgb_xgb(df_valid, size = nrow(df_valid)), obs = y_valid)
# postResample(pred = predict(randomForest, x_valid), obs = y_valid)
#
# # on the test set
# # interesting results here
# postResample(pred = py_to_r(full_xgb$predict(x_test)),obs = y_test)
# postResample(pred = infer_xgb_xgb(df_test, size = nrow(df_test)), obs = y_test)
# postResample(pred = predict(randomForest, x_test), obs = y_test)