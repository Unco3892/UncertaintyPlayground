# neural network (left) and random forest (right)
...
while (continue) {
  ...
  modL %>% compile(
    ...
    # optimizer = "adam",
    # loss = "mse",
    loss = loss_mean_absolute_error(name = "mae"),
  ...
  train_perf[[i]] <-
    postResample(pred = infer_nn_caret(df_train), obs = y_train)
  valid_perf[[i]] <-
    postResample(
      pred = infer_nn_caret(df_valid, size = valid_smp_size),
      obs = y_valid
    )
...
postResample(pred = infer_nn_caret(df_test, size = nrow(df_test)), obs = y_test)
...
postResample(
  pred = predict(randomForest, select(train_comp, -y_comp)),
  obs = train_comp$y_comp
)

postResample(
  pred = predict(randomForest, test_comp),
  obs = test_comp$y_comp
)

# fff <- data.frame(hidden_node,y_comp)
#
# sample <- sample(c(TRUE, FALSE), nrow(fff), replace=TRUE, prob=c(0.9,0.1))
# # subsetting the data
# train_comp  <- fff[sample, ]
# test_comp   <- fff[!sample, ]
