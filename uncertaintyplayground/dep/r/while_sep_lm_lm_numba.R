...
max_it <- 400
R <- 10 # 9 with `rf` worked super well and even bit the linear model
...
for (p in 1:length(n_z_outcomes)) {
  ...
  z_R_sep[[p]] <- py$sample_z_r(n_z_outcomes[p], z_means = z_t[[p]])
}


py$sample_z_r(an_z_outcomes[p], z_means = z_t[[p]])
