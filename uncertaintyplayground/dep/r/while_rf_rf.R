...
R <- 15 # 9 with `rf` worked super well and even bit the linear model
...
while (continue) {
  ...
  if (i == 1) {
    ...
    z_t <- predict(modL_p, newdata = data.frame(x_train))
    z_t <- do.call(cbind, lapply(z_t$regrOutput, "[[", "predicted"))
  }
  ...
  modL <- randomForestSRC::tune(
    ...
    # case.wt =`if`(i == 1, NULL, w_comp)
  )

  modL <- randomForestSRC::rfsrc(
    as.formula(str_c(
      "cbind(",
      paste0("X", 1:hidden, collapse = ","),
      ")~",
    ...
    data = data_left_model,
    mtry = modL$optimal[2],
    nodesize = modL$optimal[1],
    case.wt = `if`(i == 1, NULL, w_comp)
  )
  rf_mvar_pred <- predict(modL, newdata = data_left_model)
  rf_mvar_pred <- do.call(cbind, lapply(rf_mvar_pred$regrOutput, "[[", "predicted"))

  # just print it the first time as afterwards it can become disturbing
  if (i == 1) {
    cat("Left model (rfMultiVar):\n")
  }

  print(postResample(rf_mvar_pred, obs = mod_1))
  ...
  train_perf[[i]] <-
    postResample(pred = infer_rf_rf(df_train), obs = y_train)
  valid_perf[[i]] <-
    postResample(
      pred = infer_rf_rf(df_valid, size = valid_smp_size),
      obs = y_valid
    )
...
for (i in 1:length(subset_list)) {
  ...
  for (j in 1:length(model_list)) {
    ...
    if (grepl("em_lm_rf", model_names[j], fixed = TRUE)) {
      mod_pred <- infer_rf_rf(sub, size = nrow(sub))
    } else {
...
# postResample(pred = infer_rf_rf(df_valid, size = nrow(df_valid)), obs = y_valid)
...
# postResample(pred = infer_rf_rf(df_test, size = nrow(df_test)), obs = y_test)
