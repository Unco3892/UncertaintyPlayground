...
bt <- import("builtins")
# RBFKernelFn <- reticulate::py_run_string('
...
#         self._amplitude = self.add_weight(initializer=tf.zeros_initializer(), dtype=dtype, name="amplitude")
#         self._length_scale = self.add_weight(initializer=tf.zeros_initializer(), dtype=dtype, name="length_scale")
...
#             length_scale=tf.nn.softplus(2.0 * self._length_scale)
#         )
# ')$RBFKernelFn
...
#rbf_kernel_fn <- RBFKernelFn()
#print(rbf_kernel_fn$kernel)
...
# adapt the code for `Discussion: Feature Relevance` section
