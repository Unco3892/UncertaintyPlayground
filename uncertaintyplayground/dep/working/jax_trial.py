...
x = jr.uniform(key=key, minval=-5.0, maxval=5.0, shape=(n, input_dim))
f = lambda x: jnp.sin(4 * x[:, 0]) + jnp.cos(2 * x[:, 0])  # Function depends on the first predictor
signal = f(x)
y = signal + jr.normal(key, shape=signal.shape) * noise
...
xtest = jnp.linspace(-5.5, 5.5, 500).reshape(-1, 1)
xtest = jnp.tile(xtest, (1, input_dim))
...
z = jnp.linspace(-5.0, 5.0, num_inducing_points).reshape(-1, 1)
...
fig, ax = plt.subplots(figsize=(12, 5))
ax.plot(x[:, 0], y, "o", alpha=0.15, label="Training Data", color="tab:gray")
ax.plot(xtest[:, 0], meanf, label="Posterior mean", color="tab:blue")
ax.fill_between(xtest[:, 0], meanf - sigma, meanf + sigma, alpha=0.3)
[
    ax.axvline(x=z_i[0], color="black", alpha=0.3, linewidth=1)
