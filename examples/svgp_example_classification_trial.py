...
#     """
#     Stochastic Variational Gaussian Process (SVGP) Model for multi-class classification.
...
#         # Reshape to include a task dimension
#         batch_shape = mvn.batch_shape
#         latent_pred = gpytorch.distributions.MultitaskMultivariateNormal.from_batch_mvn(
#             mvn.expand(torch.Size([self.num_classes]) + batch_shape)
#         )
#         return latent_pred
...
#         inducing_points = self.X_train[:min(num_inducing_points, self.X_train.shape[0]), :]
#         self.model = SVGP(inducing_points, num_classes, dtype=self.dtype).to(
#             self.device, dtype=self.dtype)
#         self.likelihood = gpytorch.likelihoods.SoftmaxLikelihood(
#             num_features=1, num_classes=num_classes).to(self.device, dtype=self.dtype)
#     def train(self):
#         # set the seed
